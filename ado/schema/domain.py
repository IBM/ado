...
def _is_value_in_internal_range_values(
    ...
    upper: float,
    interval: float,
    window_step_count: int = 5,
) -> bool:
    """Returns True if _internal_range_values(lower, upper, interval) would generate value.
    ...
    Parameters:
        ...
        window_step_count: number of steps before and after candidate step index to check
    ...
    try:
        ...
        real_value = value.real if np.iscomplexobj(value) and value.imag == 0 else value
        if real_value < lower or real_value >= upper:
            return False

        if not is_float_range(interval=interval, domain_range=[lower, upper]):
            # Integer range: check that value falls on a grid point (offset from lower is divisible by interval)
            return bool((real_value - lower) % interval == 0)

        # Float range path: use estimation and a smaller window, then call _internal_range_values
        estimated_step_index = int(np.floor((real_value - lower) / interval))
        window_start_index = max(0, estimated_step_index - window_step_count)
        window_lower = lower + window_start_index * interval
        window_upper = min(upper, window_lower + (2 * window_step_count + 2) * interval)
        if window_lower >= window_upper:
            return False

        return value in _internal_range_values(
            lower=window_lower, upper=window_upper, interval=interval
        )
