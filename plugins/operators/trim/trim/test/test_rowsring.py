...
def test_append_and_index_reset_matches_spec():
...
def test_iadd_keeps_instance_type():
...
def test_capacity_fifo_behavior():
...
def test_columns_order_is_canonical():
...
def test_empty_df_has_known_columns_after_first_append():
    ring = RowsRing(maxlen=2)
...
def test_getitem_and_len_and_to_list():
    ring = RowsRing(maxlen=3)
    ring += {"id": 1, "value": 1.0}
    ring += {"id": 2, "value": 2.0}
    ring += {"id": 3, "value": 3.0}
...
def test_validator_hook_blocks_invalid_rows():
