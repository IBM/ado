...
def _is_value_in_internal_range_values(
    ...
    """Returns True if _internal_range_values(lower, upper, interval) would generate value without materializing the list.

    Parameters:
        value: candidate value to test
        lower: lower bound of the range (inclusive)
        upper: upper bound of the range (exclusive)
        interval: step size between values

    Returns:
        True if value would be generated by _internal_range_values, False otherwise
    """
    ...
    try:
        if real_value < lower or real_value >= upper:
            return False
    except (TypeError, ValueError):
        return False

    if not is_float_range(interval=interval, domain_range=[lower, upper]):
        # Integer range path: equivalent to `value in list(np.arange(lower, upper, interval))`
        # Must be an exact integer step from lower
        try:
            if (real_value - lower) % interval != 0:
                return False
            step_index = int((real_value - lower) // interval)
        except (TypeError, ValueError):
            return False
        expected_value = lower + step_index * interval
        return bool(value == expected_value)

    # Float range path: equivalent to `value in _internal_range_values(lower, upper, interval)`
    # Replicates the step count and rounding logic of _internal_range_values
    total_steps = int(np.floor((upper - lower) / interval)) + 1
    try:
        nearest_step_index = round((real_value - lower) / interval)
    except (TypeError, ValueError):
        return False

    # Verify the step index is within bounds [0, total_steps)
    if not (0 <= nearest_step_index < total_steps):
        return False

    # In _internal_range_values, the last item is dropped if lower + (total_steps - 1) * interval == upper
    if (
        nearest_step_index == total_steps - 1
        and (lower + nearest_step_index * interval) == upper
    ):
        return False

    expected_value = np.round(lower + nearest_step_index * interval, 10)
    return bool(value == expected_value)
