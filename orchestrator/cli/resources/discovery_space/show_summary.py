...
def show_discovery_space_summary(parameters: AdoShowSummaryCommandParameters) -> None:
    ...
    with Status(ADO_SPINNER_QUERYING_DB) as status:
        ...
        # Legacy code before using output_file in the parameters
        ...
        if (
            autogenerated_name
            and parameters.output_format == AdoShowSummarySupportedOutputFormats.CSV
        ):
            autogenerated_name.write_text(result)
            console_print(
                f"{SUCCESS} Output saved as {magenta(str(autogenerated_name))}",
                stderr=True,
            )
            return
