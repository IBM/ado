...
where `$SAMPLE_STORE_IDENTIFIER` is the identifier of the [samplestore](/ibm/ado/resources/sample-stores/#creating-a-samplestore) you want to use to store the results.
...
- *change optimizer options*: Change the optimization options and run another optimization. See [the ray tune operator documentation](/ibm/ado/operators/optimisation-with-ray-tune/) for details and further examples on what can be configured.
...
See [the documentation for custom experiments](/ibm/ado/actuators/creating-custom-experiments/) to find out more.
